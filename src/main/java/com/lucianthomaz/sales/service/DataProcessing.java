...
public class DataProcessing {
   ...
   public Result processFile(List<String> linesOfFile) {
      ...
      if (!linesOfFile.isEmpty()) {
         ...
         if (!salesmen.isEmpty() && !clients.isEmpty() && !sales.isEmpty()) {
   ...
   private List<Salesman> getSalesmen(List<String> lines) {
      ...
      return lines.stream().filter(x-> x.startsWith(DataIdentifierEnum.SALESMAN_ID.getId()))
   ...
   private List<Client> getClients(List<String> lines) {
      ...
      return lines.stream().filter(x-> x.startsWith(DataIdentifierEnum.CLIENT_ID.getId()))
            .map(x -> x.split(DelimiterEnum.COLUMN_DELIMITER.getDelimiter()))
   ...
   private List<Sale> getSales(List<String> lines, List<Salesman> salesmen) {
      if (!salesmen.isEmpty()) {
         ...
         return lines.stream().filter(x-> x.startsWith(DataIdentifierEnum.SALE_ID.getId()))
               .map(x -> x.split(DelimiterEnum.COLUMN_DELIMITER.getDelimiter()))
